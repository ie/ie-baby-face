import GothamBoldEOT from '@/fonts/Gotham-Bold.eot'
import GothamBoldSVG from '@/fonts/Gotham-Bold.svg'
import GothamBoldTTF from '@/fonts/Gotham-Bold.ttf'
import GothamBoldWOFF from '@/fonts/Gotham-Bold.woff'
import GothamBoldWOFF2 from '@/fonts/Gotham-Bold.woff2'
import GothamBoldItalicEOT from '@/fonts/Gotham-BoldItalic.eot'
import GothamBoldItalicSVG from '@/fonts/Gotham-BoldItalic.svg'
import GothamBoldItalicTTF from '@/fonts/Gotham-BoldItalic.ttf'
import GothamBoldItalicWOFF from '@/fonts/Gotham-BoldItalic.woff'
import GothamBoldItalicWOFF2 from '@/fonts/Gotham-BoldItalic.woff2'
import GothamBookEOT from '@/fonts/Gotham-Book.eot'
import GothamBookSVG from '@/fonts/Gotham-Book.svg'
import GothamBookTTF from '@/fonts/Gotham-Book.ttf'
import GothamBookWOFF from '@/fonts/Gotham-Book.woff'
import GothamBookWOFF2 from '@/fonts/Gotham-Book.woff2'
import GothamBookItalicEOT from '@/fonts/Gotham-BookItalic.eot'
import GothamBookItalicSVG from '@/fonts/Gotham-BookItalic.svg'
import GothamBookItalicTTF from '@/fonts/Gotham-BookItalic.ttf'
import GothamBookItalicWOFF from '@/fonts/Gotham-BookItalic.woff'
import GothamBookItalicWOFF2 from '@/fonts/Gotham-BookItalic.woff2'

export default {
  GothamBoldEOT,
  GothamBoldSVG,
  GothamBoldTTF,
  GothamBoldWOFF,
  GothamBoldWOFF2,
  GothamBoldItalicEOT,
  GothamBoldItalicSVG,
  GothamBoldItalicTTF,
  GothamBoldItalicWOFF,
  GothamBoldItalicWOFF2,
  GothamBookEOT,
  GothamBookSVG,
  GothamBookTTF,
  GothamBookWOFF,
  GothamBookWOFF2,
  GothamBookItalicEOT,
  GothamBookItalicSVG,
  GothamBookItalicTTF,
  GothamBookItalicWOFF,
  GothamBookItalicWOFF2,
}
